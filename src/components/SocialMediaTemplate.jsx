...
export default function SocialMediaTemplate({ businessName, palette, typography, svgDataUrl, style, onCanvasReady }) {
  ...
  return (
    <div className="space-y-4 animate-on-mount stagger-5">
      {/* Section Header */}
      <div className="flex items-center gap-3 px-2">
        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-pink-500/20 to-rose-500/20 border border-pink-500/30 flex items-center justify-center text-pink-400 text-sm shadow-[0_0_10px_rgba(236,72,153,0.1)]">
          📱
        </div>
        <div className="flex items-center gap-3 flex-1">
          <h2 className="text-xl font-bold text-slate-100 tracking-tight">Social Post</h2>
          <span className="text-[10px] font-bold text-slate-400 bg-slate-800/50 border border-slate-700/50 px-2.5 py-1 rounded-md uppercase tracking-wider">
            1080×1080
          </span>
        </div>
      </div>
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8">
        ...
        <div className="mb-6 bg-slate-900/60 p-4 sm:p-6 rounded-2xl border border-slate-700/50 shadow-inner relative overflow-hidden group max-w-md mx-auto">
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5 pointer-events-none"></div>
          
          <canvas
            ref={canvasRef}
            className={`w-full h-auto rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.4)] ring-1 ring-white/10 transition-all duration-700 ${isRendered ? 'opacity-100 transform translate-y-0 scale-100' : 'opacity-0 transform translate-y-4 scale-95'}`}
          />
          
          {!isRendered && (
            <div className="h-64 flex flex-col items-center justify-center text-slate-500 bg-slate-800/20 rounded-xl relative z-10">
              <svg className="animate-spin h-6 w-6 mb-3 text-pink-500/50" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"/>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
              </svg>
              <span className="text-xs font-semibold uppercase tracking-widest text-slate-400">Rendering Post...</span>
            </div>
          )}
        </div>
